    ...
    <div style={{position:"absolute",bottom:"calc(100% + 8px)",left:0,background:"#fff",border:"1px solid #DBDBDB",borderRadius:12,padding:10,display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:4,zIndex:200,boxShadow:"0 4px 20px rgba(0,0,0,0.15)",width:216}}>
      {EMOJIS.map(e=>(
        <button key={e} onClick={()=>{onPick(e);onClose();}}
          style={{background:"none",border:"none",fontSize:20,cursor:"pointer",padding:4,borderRadius:6,lineHeight:1,transition:"background 0.1s"}}
          onMouseEnter={ev=>ev.target.style.background="#F5F5F5"}
...
function PostCard({post,currentUser,onDelete,onNavigate}) {
  ...
  useEffect(()=>{ getAuthor(post.userId).then(u=>u&&setAuthor(u)); },[post.userId]);

  // Fetch real comment count on mount — post.commentCount from backend is often stale/0
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    // Real likeCount bhi fetch karo — post.likeCount stale ho sakta hai
  ...
  const toggleCmts=async()=>{
    ...
    if(opening&&cmts.length===0){
      setLoadingCmts(true);
      try{ const d=await commentApi.getByPost(post.postId); const list=Array.isArray(d)?d:[]; setCmts(list); setCmtCount(list.length); }
      catch{ setCmts([]); }
  ...
  const handleComment=async()=>{
    ...
    } catch(e){ setCmtErr(e.message||"Comment failed"); }
  ...
  return (
    <div style={{background:"#fff",border:"1px solid #DBDBDB",borderRadius:12,overflow:"hidden",marginBottom:16,boxShadow:"0 1px 4px rgba(0,0,0,0.07)"}}>
      ...
      <div style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:10}}>
        <div style={{display:"flex",alignItems:"center",gap:10,flex:1,cursor:"pointer"}} onClick={()=>onNavigate("profile",post.userId)}>
          {author?.avatarUrl
          ...
          <div>
            ...
            <div style={{fontSize:12,color:"#A8A8A8"}}>{authorHandle}{authorHandle?" · ":""}{timeAgo(post.createdAt)}</div>
          </div>
        </div>
        {post.userId===currentUser?.userId&&(
          <button onClick={()=>onDelete(post.postId)} style={{background:"none",border:"none",color:"#A8A8A8",cursor:"pointer",fontSize:16,padding:"4px 6px",borderRadius:6}} title="Delete">🗑️</button>
        )}
      </div>
      ...
      {post.hashtags&&(
        <div style={{padding:"0 14px 10px",display:"flex",gap:6,flexWrap:"wrap"}}>
          {post.hashtags.split(",").filter(Boolean).map(t=>(
            <span key={t} style={{fontSize:13,color:"#0095F6",cursor:"pointer"}}>{t.startsWith("#")?t:`#${t}`}</span>
          ))}
        </div>
      )}
      ...
      {post.mediaUrl&&post.mediaType==="IMAGE"&&(
        <img src={post.mediaUrl} alt="post" style={{width:"100%",maxHeight:500,objectFit:"cover",display:"block"}}/>
      )}

      {/* Like error */}
      {likeErr&&<div style={{padding:"4px 14px",fontSize:12,color:"#ED4956"}}>⚠️ {likeErr}</div>}

      {/* Actions */}
      <div style={{display:"flex",padding:"8px 10px 4px",gap:2,borderTop:"1px solid #DBDBDB",marginTop:4}}>
        <button onClick={handleLike} disabled={likeLoading}
          style={{display:"flex",alignItems:"center",gap:6,padding:"6px 10px",background:"none",border:"none",cursor:"pointer",borderRadius:8,color:liked?"#ED4956":"#262626",fontWeight:liked?700:400,fontSize:13,transition:"all 0.1s"}}>
          ...
          {cmts.map(c=>(
            <div key={c.commentId} style={{display:"flex",gap:8}}>
              ...
              <div style={{background:"#F5F5F5",borderRadius:10,padding:"8px 12px",flex:1}}>
                <span style={{fontSize:13,fontWeight:700}}>{_uc[c.userId]?.userName ?? _uc[c.userId]?.fullName ?? `User ${c.userId}`}</span>
  ...
  useEffect(()=>{
    ...
    (async()=>{
      try{
        let data=[];
        try{
          data=await postApi.getPublic(1,50);
          // Deleted users ki posts filter karo — userId fetch karo, 404 = deleted
          const validPosts = await Promise.all(
            (Array.isArray(data)?data:[]).map(async post => {
              const exists = await userApi.exists(post.userId);
              return exists ? post : null;
            })
          );
          data = validPosts.filter(Boolean);
        } catch(e){
          console.error("[Feed] getPublic failed:", e);
          data=[];
        }
  ...
  const handlePost=async()=>{
    ...
    try{
      const newPost=await postApi.create({
        ...
        hashtags:hashtags.trim()||"", visibility:"PUBLIC",
      });
      ...
      <div style={{padding:"24px 20px",maxWidth:1100}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 300px",gap:32,alignItems:"start"}}>
          <div>
            ...
            <div style={{background:"#fff",border:"1px solid #DBDBDB",borderRadius:12,marginBottom:16,overflow:"visible"}}>
              <div style={{display:"flex",gap:12,alignItems:"flex-start",padding:"14px 14px 10px"}}>
                <div style={{width:42,height:42,borderRadius:"50%",background:getG(user?.userId),display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:14,flexShrink:0,cursor:"pointer"}}
                  onClick={()=>onNavigate("profile",null)}>{userInitials}</div>
                <textarea
                  ...
                  value={postText} onChange={e=>{setPostText(e.target.value);setError("");}}/>
              </div>
              ...
              {mediaPreview&&(
                <div style={{position:"relative",margin:"0 14px 8px"}}>
                  <img src={mediaPreview} alt="preview" style={{width:"100%",maxHeight:280,objectFit:"cover",borderRadius:8}}/>
                  <button onClick={()=>{setMediaPreview(null);setMediaUrl("");setMediaType("NONE");}}
                    style={{position:"absolute",top:8,right:8,background:"rgba(0,0,0,0.65)",color:"#fff",border:"none",borderRadius:"50%",width:28,height:28,cursor:"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
                </div>
              )}
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px 12px",borderTop:"1px solid #F0F0F0"}}>
                <div style={{display:"flex",gap:4,alignItems:"center"}}>
                  ...
                  <button onClick={()=>fileRef.current.click()}
                    style={{background:"none",border:"none",cursor:"pointer",padding:"6px 8px",borderRadius:8,display:"flex",alignItems:"center",color:"#737373"}} title="Add Photo">
                  ...
                  <button onClick={()=>setHashtags(h=>h?h:"#")}
                    style={{background:"none",border:"none",cursor:"pointer",padding:"6px 10px",borderRadius:8,fontSize:15,fontWeight:700,color:"#737373"}} title="Add Hashtag">#</button>
                  {/* Emoji */}
                  <div ref={emojiRef} style={{position:"relative"}}>
                    <button onClick={()=>setShowEmoji(e=>!e)}
                      style={{background:"none",border:"none",cursor:"pointer",padding:"4px 8px",borderRadius:8,fontSize:20}} title="Add Emoji">😊</button>
                    {showEmoji&&(
                      <EmojiPicker onPick={e=>setPostText(t=>t+e)} onClose={()=>setShowEmoji(false)}/>
                    )}
                  </div>
                </div>
                {/* FIX: disabled agar content nahi */}
                <button onClick={handlePost} disabled={!canPost}
                  style={{background:canPost?"#0095F6":"#B2DFFC",color:"#fff",border:"none",borderRadius:8,padding:"8px 20px",fontSize:14,fontWeight:700,cursor:canPost?"pointer":"not-allowed",transition:"background 0.15s"}}>
                  {posting?"Posting…":"Share Post"}
                </button>
              </div>
            </div>

            {(authLoading||loading)&&<div style={{textAlign:"center",padding:40,color:"#737373"}}>Loading feed…</div>}
            {!authLoading&&!loading&&posts.length===0&&(
              <div style={{textAlign:"center",padding:"48px 0"}}>
                ...
                <div style={{fontSize:14,color:"#737373",marginBottom:16}}>Follow people to see their posts</div>
            ...
            {!authLoading&&!loading&&posts.map(post=>(
              <PostCard key={post.postId} post={post} currentUser={user}
                onDelete={async(id)=>{ try{await postApi.delete(id);setPosts(p=>p.filter(x=>x.postId!==id));}catch(e){setError(e.message);} }}
          ...
          <div style={{position:"sticky",top:80}}>
            <div style={{background:"#fff",border:"1px solid #DBDBDB",borderRadius:12,padding:16,marginBottom:16}}>
              <div style={{fontWeight:700,fontSize:14,marginBottom:12}}>🔥 Trending</div>
              {["#ConnectSphere","#WebDev","#TechIndia","#CleanCode","#ReactJS"].map((t,i)=>(
                <div key={t} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",cursor:"pointer"}}
                  onClick={()=>onNavigate("explore",null,t)}>
                  <span style={{fontSize:12,color:"#A8A8A8",fontWeight:700,width:16}}>#{i+1}</span>
                  <div style={{fontWeight:600,fontSize:14,color:"#0095F6"}}>{t}</div>
                </div>
              ))}
            </div>
            <div style={{fontSize:11,color:"#A8A8A8",lineHeight:1.8}}>ConnectSphere · © 2026</div>
          </div>
        </div>
      </div>
    </div>
